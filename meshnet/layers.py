...
class FaceKernelCorrelation(nn.Module):
    ...
    def forward(self, normals, neighbor_index):
        b, _, n = normals.size() # [b, 3, n]

        center = normals.unsqueeze(2).expand(-1, -1, self.num_kernel, -1).unsqueeze(4) # [b, 3, k, n, 1]
        neighbor = torch.gather(normals.unsqueeze(3).expand(-1, -1, -1, 3), 2,         # [b, 3, n, 3]
                                neighbor_index.unsqueeze(1).expand(-1, 3, -1, -1))     # [b, 3, n, 3]
        neighbor = neighbor.unsqueeze(2).expand(-1, -1, self.num_kernel, -1, -1)       # [b, 3, n, 3]->
                                                                                       # [b, 3, k, n, 3]
        fea = torch.cat([center, neighbor], 4) # [b, 3, k, n, 4]
        fea = fea.unsqueeze(5).expand(-1, -1, -1, -1, -1, 4) # [b, 3, k, n, 4, 4]
        weight = torch.cat([torch.sin(self.weight_alpha) * torch.cos(self.weight_beta),
                            ...
                            torch.cos(self.weight_alpha)], 0) # [3, k, 4]
        weight = weight.unsqueeze(0).expand(b, -1, -1, -1) # [b, 3, k, 4]
        weight = weight.unsqueeze(3).expand(-1, -1, -1, n, -1) # [b, 3, k, n, 4]
        weight = weight.unsqueeze(4).expand(-1, -1, -1, -1, 4, -1) # [b, 3, k, n, 4, 4]
        dist = torch.sum((fea - weight)**2, 1) # [b, k, n, 4, 4]
        fea = torch.sum(torch.sum(np.e**(dist / (-2 * self.sigma**2)), 4), 3) / 16 # [b, k, n]
...
class StructuralDescriptor(nn.Module):
    ...
    def forward(self, corners, normals, neighbor_index):
        structural_fea1 = self.FRC(corners) # [b, 64, n]
        structural_fea2 = self.FKC(normals, neighbor_index) # [b, k, n]
...
class MeshConvolution(nn.Module):
    ...
    def forward(self, spatial_fea, structural_fea, neighbor_index):
        b, _, n = spatial_fea.size() # [b, in1, n]
        # Combination
        spatial_fea = self.combination_mlp(torch.cat([spatial_fea, structural_fea], 1))
        # Aggregation
        if self.aggregation_method == 'Concat':#[b, in2, n, 3], [b, in2, n, 3]
            structural_fea = torch.cat([structural_fea.unsqueeze(3).expand(-1, -1, -1, 3),\
                torch.gather(structural_fea.unsqueeze(3).expand(-1, -1, -1, 3), 2,\
                    neighbor_index.unsqueeze(1).expand(-1, self.structural_in_channel,-1, -1))], 1)
            structural_fea = self.concat_mlp(structural_fea) #[b, in2, n, 3]
            structural_fea = torch.max(structural_fea, 3)[0] #[b, in2, n]
        elif self.aggregation_method == 'Max': #[b, in2, n, 1]
            structural_fea = torch.cat([structural_fea.unsqueeze(3),\
                torch.gather(structural_fea.unsqueeze(3).expand(-1, -1, -1, 3), 2,\
                    neighbor_index.unsqueeze(1).expand(-1, self.structural_in_channel,-1, -1))], 3)
